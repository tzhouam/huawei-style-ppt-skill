---
name: ppt-slide-authoring
description: PPT HTML 制作规范 — 混合布局、Pre-flight Checklist 自检、愿景优先原则、素材使用规范。C 场景详细流程。
---

# 02 · Slide 制作（C 场景）

> 触发：A 场景大纲确认 + B 场景风格确认后，开始画 slide。
...
```
1. 读证据源，提取核心数据和论点
2. 选 archetype 模板（见 references/density-playbook.md 页面构成模板）
3. 画 HTML（1280×720 fixed viewport）
4. 跑 Pre-flight Checklist（见下方）
5. 截图自检（必须自己看一遍渲染结果）
6. 通过 → 进入 D 场景视觉审查（03-visual-review.md）
   不通过 → 改到通过
```
...
- [ ] 渲染截图已保存？（教训：自检必须截图看一遍再交活）

> 自检通过后，进入 D 场景视觉审查（见 [03-visual-review.md](03-visual-review.md)）。注意：D 场景 reviewer 会做 Archetype Guard 和 Regression Pair Gate 两道额外门禁，提前意识到有利于减少返工。

## 素材使用

### 图片 / 截图

- **优先用真实产出**（教程素材、产品截图、真实 dashboard）
- 一张截图 = 几十行文字的信息量，是密度性价比最高的手段
- 图片太大时用 `max-height` 限制，不裁剪内容
- 图片路径注意 HTTP server 根目录（不要用跳出 server root 的相对路径）
- CSS 模板见 [references/density-playbook.md 截图 + Callout 标注 CSS 模式](references/density-playbook.md#截图--callout-标注-css-模式)

### SmartArt / 流程图

- 用 CSS flexbox + `▶` 字符实现横向箭头链
- 交替色增加视觉区分
- CSS 模板见 [references/density-playbook.md SmartArt/流程图 HTML 模板](references/density-playbook.md#smartart流程图-html-模板)
...
4. **独立思考 > 迎合 review** — 每个角色都要有独立判断

> 血泪教训：D5 中 D1 reviewer 说"0 生产事故不在单一证据源" → author 直接删了有冲击力的 KPI。
